...
| **分类粒子特效** | 169 个骰子逐一配粒子，覆盖「玩家实体（持续附着/环绕/痕迹）、射击战斗（枪口/击杀/击杀者/爆头/伤害反馈）、回合事件（触发/回合开始/回合结束）、UI（HUD 面板/粒子预览菜单）」四类；**稀有度越高覆盖类别越多**。持续类均用每 tick 跟随的独立粒子，**不挂常驻光环** |
...
| `FinalJudgment` | 坠落天空 | 按 E 落点长出苍白穹顶、高空铺开法阵群，20s 后贯下通天光柱；落点 50m 内 2000→250 衰减，越近越白，CD 90s |
...
```jsonc
{
  "effects": {
    ...
    "trails": true,   // false = 只关「持续类」（脚底痕迹 / 持续附着 / 环绕），保留触发类
    "hud": true       // 抽到骰子时在屏幕中央显示 HUD 面板（骰子名 + 档位 + 覆盖的特效类别）
  }
}
```

每个骰子用哪种粒子写在源码 `src/RollTheDice.Utils/DiceEffects.cs` 的设计表里（一行一个骰子），
不需要、也不要改各个骰子文件。特效按类别组织，**稀有度越高覆盖的类别越多**：

| 类别 | 字段 | 时机 | 例子 |
|------|------|------|------|
| 玩家实体 · 持续附着 | `hold` | 持续贴在玩家身上（每 tick 跟随，不挂 Parent） | 护盾 / 圣骑士 → 护甲辉光 |
| 玩家实体 · 环绕 | `orbit` | 绕玩家旋转（每 tick 跟随） | 神 / 狼王 → 环绕光环 |
| 玩家实体 · 脚底痕迹 | `trail` | 周期性在**脚底**留下小粒子 | 火魔 → 小火余烬；毒刃 → 孢子；幽灵 → 雾气 |
| 射击战斗 · 枪口/弹道 | `onFire` | 开火时（0.05s 节流；跳过刀/雷/C4） | 枪械骰 → 枪口火花 |
| 射击战斗 · 击杀 | `onKill` | 击杀敌人在**尸体处** | 火龙 → 火焰覆盖 |
| 射击战斗 · 击杀者 | `onKillSelf` | 击杀时在**自己**身上 | 狙击精英 → 弹壳 |
| 射击战斗 · 爆头 | `onKillHeadshot` | **爆头**击杀（否则退回 `onKill`） | 传说骰 → 爆头血花 |
| 射击战斗 · 阵亡 | `onDeath` | 自己阵亡 | 凤凰 → 浴火 |
| 射击战斗 · 伤害反馈 | `onHurt` / `onHit` | 自己受击 / 命中敌人（各 0.25s 节流） | 护甲跳弹 |
| 射击战斗 · 爆头标记 | `onHitHeadshot` | 命中头部（否则退回 `onHit`） | 爆头血花 |
| 回合与事件 | `burst` / `roundStart` / `roundEnd` / `remove` | 抽到 / 回合开始 / 回合结束 / 效果结束 | 传说骰 → 全类别覆盖 |

**稀有度决定覆盖范围，以及是否「特调」**：

- **普通**：只有 `burst`。
- **稀有**：+ `onKill`（这两个档位按主粒子 theme 自动补，属大众化）。
- **史诗 / 传说 / 组合**：**逐条特调** —— 每个骰子的**每个事件都单独挑选**贴合该骰子语义的粒子（不做 theme 自动填充），并覆盖持续附着/环绕/枪口/击杀/击杀者/爆头/阵亡/伤害反馈/爆头标记/回合开始/回合结束。

> 例如「上帝」= 金色光环射线 + 金色爆闪 + 护甲辉光；「诸神黄昏」= 风暴闪电 / 电光 / 电弧；「虚空」= 幽灵雾气；「核泄漏」= 危险区 + 放射性余烬；「蚊子/饕餮」= 血；「尤格/奇点」= 黑烬 + 扭曲。同为传说也不会撞同一套粒子。

> **不再使用「常驻光环」**：旧版几乎每个骰子都往玩家身上挂一个常驻粒子（火系全是 `env_fire_large`），
> 看起来就是一团火挂在身上、又丑又挡视线。现在持续类特效都用「每 tick 重新定位的独立粒子实体」实现，
> 只给盾/护盾类（`hold`）和传说/组合（`orbit`）等确有语义的骰子使用，且不会有挂 Parent 的未知行为。

**粒子菜单（预览各类特效）**：管理员可用 `!rtdeffect <preset>` 预览类别，`preset` 取值：
`muzzle` / `tracer` / `kill` / `killer` / `headshot` / `hurt` / `hit` / `aura` / `orbit` / `trail` / `roundstart` / `roundend` / `death`；
也可 `!rtdeffect <particles/....vpcf> [self|crosshair|near] [秒]` 直接播放任意粒子。
