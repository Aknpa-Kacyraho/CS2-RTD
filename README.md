...
**按档位自动补齐**（设计表里只写"主粒子 theme" + 需要的显式覆盖，其余自动补）：

- 普通：只有 `burst`。
- 稀有：+ `onKill`。
- 史诗：+ `onKillSelf` / `onDeath` / `roundStart` / `onHurt` / `onHit`（伤害反馈用通用护甲火花）。
- 传说 / 组合：+ `onKillHeadshot` / `onHitHeadshot` / `roundEnd`，多数还有 `orbit` 环绕。
